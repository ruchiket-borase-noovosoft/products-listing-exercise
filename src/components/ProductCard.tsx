export default function ProductCard ({product} : {product: ProductType}) {
    ...
    return(
        <div className="relative flex flex-col group gap-2 cursor-pointer items-center p-4 shadow-sm rounded-2xl md:max-w-[280px]">
            <span className="top-3 left-3 absolute bg-red-300 opacity-80 text-xs font-medium p-1 text-[10px] px-2 rounded-full">{product.discountPercentage}% OFF</span>
            ...
            <div className="w-full w-full flex items-center justify-between ">
                <div><h1  className={`text-xl font-bold`}>${discountedPrice.toFixed(2)}</h1>
